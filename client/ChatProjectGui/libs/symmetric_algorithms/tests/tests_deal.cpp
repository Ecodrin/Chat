...
void tests() {
    ...
    std::vector<size_t> deal_key_size = {16, 24,  32};

    int total = 0, passed = 0;

    for(size_t key_size: deal_key_size) {
        std::cout << "-----------------------------" << "\nDEAL" << key_size * 8 << "\n-----------------------------" << std::endl;
        for (auto mode : modes) {
            for (auto pad : paddings) {
                for (size_t len : lengths) {
                    ...
                    total++;
                    if (test_in_memory(plaintext, mode, pad, key_size)){ 
                        passed++;
                    } else {
                    ...
                    total++;
                    if (test_file(plaintext, mode, pad, key_size)) {
                        passed++;
                    } else {
...
void check_file() {
    ...
    std::cout << "Iv: ";
    bytes_utility::print_bytes_vector(std::cout, iv);
    ...
    std::string s = ".jpg";
    std::string input_file_name = "/mnt/c/Users/Матвей/Documents/Unik/labs_C/sem5/CO/libs/symmetric_algorithms/tests/yarik";
